...
function selectDropdownItem() {
    ...
    if (scmListItem) {
        ...
        setTimeout(selectKitRequest, 1000);
    }
}

function selectKitRequest() {
    ...
    if (issueTypeField) {
        issueTypeField.value = 'Kit Request';
        ...
        setTimeout(() => {
            const selector = 'li.aui-list-item-li-kit-request';
        ...
        setTimeout(selectKitRequest, 500);
    }
}
...
function setInputValue() {
    ...
    if (textBox) {
        ...
        textBox.value = "Kit request for " + manifestName + " for build - " + buildNumber;
    }
...
function setInputForTinyMCE() {
    ...
    if (iframe) {
        ...
        if (pElement) {
            ...
            pElement.innerHTML = "Kit Name: " + manifestName + "<br>" + "Version Number: " + versionNumber + "<br>" + "Part Number: " + partNumber;
            // Trigger the 'input' and 'change' events for TinyMCE to detect the change
            const event = new Event('input', {
                bubbles: true,
                cancelable: true,
            });
            pElement.dispatchEvent(event); // For input event
            pElement.dispatchEvent(new Event('change', { bubbles: true })); // For change event

            console.log("Updated the TinyMCE input field with:", pElement.innerHTML);
...
function setInputValue2() {
    ...
    if (textBox) {
        console.log("Filling ATP number: ", atpData);
...
function setDateSevenDaysFromNow() {
    ...
    const dateInput2 = document.getElementById('customfield_13503');
...
function selectKitTest() {
    ...
    if (select) {
        ...
        for (let i = 0; i < options.length; i++) {
            if (options[i].text === 'Kit - Formal') {
                select.selectedIndex = i;
                console.log("Selected 'Kit - Formal'");
                break;
            }
        }
    } else {
